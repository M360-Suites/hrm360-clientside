...
const Topbar = ({ onMenuClick }: TopbarProps) => {
  ...
  return (
    <header className="h-16 sm:h-20 bg-white border-b border-gray-100 flex items-center justify-between gap-3 px-3 sm:px-8 shrink-0">
      <div className="flex min-w-0 items-center gap-2 sm:gap-4">
        ...
        <h1 className="text-lg sm:text-xl font-bold text-gray-900 truncate max-w-[55vw] sm:max-w-none">
          {formattedTitle}
        </h1>
      </div>
        ...
        <div className="relative sm:pl-6 sm:border-l border-gray-100" ref={dropdownRef}>
          ...
          >
            <div className="text-right hidden md:block">
              ...
              <p className="text-[10px] font-bold text-[#4A1D96] uppercase tracking-wider">{user?.role || "Administrator"}</p>
            </div>
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-[#4A1D96] overflow-hidden shadow-xs shadow-indigo-500/5">
              {user?.image
