  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      <div className="flex items-end justify-between">
        <div>
          <h2 className="text-xl font-semibold text-gray-800 mb-1">Attendance Tracking</h2>
          <p className="text-sm text-gray-500">Real-time presence management for your organization</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Current Time</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm shadow-indigo-500/5">
          <p className="text-xs font-bold text-indigo-500 uppercase tracking-wider mb-2">Present Today</p>
          <h3 className="text-3xl font-bold text-gray-900">{todayStats?.present || 0}</h3>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm shadow-amber-500/5">
          <p className="text-xs font-bold text-amber-500 uppercase tracking-wider mb-2">Late Arrivals</p>
          <h3 className="text-3xl font-bold text-gray-900">{todayStats?.late || 0}</h3>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm shadow-rose-500/5">
          <p className="text-xs font-bold text-rose-500 uppercase tracking-wider mb-2">Absent Today</p>
          <h3 className="text-3xl font-bold text-gray-900">{todayStats?.absent || 0}</h3>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm shadow-emerald-500/5">
          <p className="text-xs font-bold text-emerald-500 uppercase tracking-wider mb-2">Attendance Rate</p>
          <h3 className="text-3xl font-bold text-gray-900">{todayStats?.rate || '0%'}</h3>
        </div>
      </div>
              ...
              <table className="w-full text-left text-sm text-gray-600 whitespace-nowrap">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredEmployees.map((emp: any) => {
                    const isClockedIn = dayAttendance.some(record => 
                      (record.employeeId === emp._id || record.employeeId === emp.id) && !record.clockOut
                    );
                    
                    return (
                      <tr key={emp._id || emp.id} className="hover:bg-gray-50/30 transition-colors">
                        ...
                        <td className="px-6 py-4 text-xs font-medium text-gray-500">{emp.role}</td>
                        <td className="px-6 py-4 text-right">
                          {isClockedIn ? (
                            <button 
                              onClick={() => handleClockOut(emp._id || emp.id)}
                            ...
                            <button 
                              onClick={() => handleClockIn(emp._id || emp.id)}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-4 max-h-[500px] overflow-y-auto">
              {dayAttendance.length > 0 ? dayAttendance.map((record: any, idx: number) => (
                <div key={record.id || idx} className="flex gap-3 p-3 rounded-xl bg-gray-50/50 border border-gray-100">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                    record.clockOut ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'
                  }`}>
                    <Clock size={14} />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-gray-900 truncate">{record.employeeName || 'Employee'}</p>
                    <p className="text-[10px] text-gray-500">
                      {record.clockOut ? `Out: ${record.clockOut}` : `In: ${record.clockIn}`}
                    </p>
                  </div>
