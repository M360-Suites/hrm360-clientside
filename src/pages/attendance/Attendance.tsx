...
const Attendance = () => {
  ...
  const hasClockedIn = Boolean(
    clockInTimeValue || employeeToday?.status === "Present",
  );

  const hasClockedOut = Boolean(clockOutTimeValue || employeeToday?.checkedOut);
  ...
  const handleQrSubmit = useCallback(
    async (value: {
      ...
      if (
        ...
        setScanMessage(
          "Location data is required for attendance. Please allow location access and try again.",
        );
      ...
      <div className="w-full max-w-5xl mx-auto pb-12">
        ...
        <div className="rounded-[1.5rem] border border-slate-100 bg-white p-4 shadow-sm sm:rounded-[2rem] sm:p-6">
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-4">
            ...
            {location &&
              // ensure latitude/longitude are numbers before calling toFixed
