...
const Settings = () => {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 pb-12">
      ...
      <div>
        <p className="mb-1 text-xs font-semibold uppercase tracking-[0.18em] text-[#4A1D96]">Workspace preferences</p>
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="h-fit rounded-3xl border border-gray-100 bg-white p-3 shadow-xs">
          <div className="mb-3 rounded-2xl bg-linear-to-br from-[#4A1D96] to-[#8B5CF6] p-4 text-white">
          ...
          <nav className="space-y-1">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              return <button key={tab.id} onClick={() => tab.id === "users" ? openUsers() : setActiveTab(tab.id)} className={"flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-medium transition " + (activeTab === tab.id ? "bg-indigo-50 text-[#4A1D96]" : "text-gray-600 hover:bg-gray-50")}><Icon size={18} /><span className="flex-1">{tab.label}</span><ChevronRight size={15} /></button>;
            })}
          </nav>
        </aside>
...
const LoadingPanel = () => <div className="flex min-h-96 items-center justify-center rounded-3xl border border-gray-100 bg-white text-[#4A1D96]"><Loader2 className="animate-spin" size={28} /></div>;
...
const Toggle = ({ checked, onChange }: { checked: boolean; onChange: (checked: boolean) => void }) => <button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)} className={"relative h-7 w-12 rounded-full transition " + (checked ? "bg-[#4A1D96]" : "bg-gray-200")}><span className={"absolute top-1 h-5 w-5 rounded-full bg-white shadow transition " + (checked ? "left-6" : "left-1")} /></button>;
...
const ActionButton = ({ label, loading, primary, type = "button", onClick }: { label: string; loading: boolean; primary?: boolean; type?: "button" | "submit"; onClick?: () => void }) => <button type={type} onClick={onClick} disabled={loading} className={"inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition disabled:opacity-50 " + (primary ? "bg-[#4A1D96] text-white hover:bg-[#8B5CF6]" : "border border-gray-200 text-gray-700 hover:bg-gray-50")}>{loading && <Loader2 className="animate-spin" size={16} />}{label}</button>;
const SaveBar = ({ onSave, saving }: { onSave: () => void; saving: boolean }) => <div className="mt-6 flex justify-end border-t border-gray-100 pt-5"><button onClick={onSave} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[#4A1D96] px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">{saving ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />}Save changes</button></div>;
