...
const Probation = () => {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="mb-1 text-xs font-semibold uppercase tracking-[0.18em] text-[#3B00D9]">Employee confirmation</p><h2 className="text-2xl font-semibold text-gray-900">Probation</h2><p className="mt-1 text-sm text-gray-500">Track objectives, review progress, and make confirmation decisions.</p></div><button onClick={() => { clearError(); setShowAdd(true); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#3B00D9] px-5 py-3 text-sm font-semibold text-white"><Plus size={17} />Add probation staff</button></div>
      ...
      <section className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-xs"><div className="flex flex-col gap-3 border-b border-gray-100 p-4 sm:flex-row sm:p-5"><label className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} className={inputClass + " pl-10"} placeholder="Search name, email, role, or work mode" /></label><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className={inputClass + " sm:w-52"}><option>All</option><option>On probation</option><option>Extended</option><option>Confirmed</option><option>Terminated</option></select></div>{isLoading ? <div className="flex min-h-72 items-center justify-center text-[#3B00D9]"><Loader2 className="animate-spin" size={28} /></div> : filteredStaff.length ? <div className="divide-y divide-gray-100">{filteredStaff.map((item, index) => <ProbationRow key={probationId(item) || index} staff={item} loading={activeAction === "details-" + probationId(item)} onView={() => void openDetails(item)} />)}</div> : <EmptyState />}</section>
