...
export const Verification = () => {
  ...
  const { onboardingSteps, error, code, loading, completeOnboarding } =
    useOnboardingStore();
  ...
  const isDisabled =
    onboardingSteps === 1 ? code.trim().length < 5 : selectedTeam.name === "";
  ...
  return (
    <div
      className={`flex flex-col ${onboardingSteps === 1 ? "gap-21 max-md:gap-6" : "gap-10 max-md:gap-6"} sm:pb-20 w-full justify-center items-center max-md:pt-10 max-md:relative`}
    >
      <div className="flex flex-col gap-3 max-md:gap-2 px-5 max-md:px-0 self-start max-w-133 max-md:w-full max-md:self-start">
        <h1 className="text-4xl/[100%] max-md:text-2xl/[120%] sm:text-base font-semibold">
          {currentStep?.title}
        </h1>
        <div className="flex items-center flex-wrap gap-1">
          <p className="text-[#4A4A4A] text-lg max-md:text-sm font-normal">
      ...
      <div>
        {currentStep?.step === 1 ? (
          <div className="flex flex-col gap-6 max-md:gap-4 px-5 justify-center items-center w-full max-md:w-full">
            <CustomInputOtp />
          </div>
        ) : (
          <div className="flex flex-col gap-5 justify-center items-start w-133">
            {teamData.map((data, index) => (
              <button
                key={index}
                className={`flex items-center cursor-pointer gap-2 p-6 border rounded-lg w-md ${selectedTeam.name === data.name && "border-[#4C0BFD]"}`}
                onClick={() => {
                  handleTeamSelect(data);
                }}
              >
                <div
                  className={`h-4 w-4 rounded-full border flex items-center justify-center ${selectedTeam.name === data.name && "border-[#4C0BFD]"}`}
                >
                  <div
                    className={`h-2.5 w-2.5 rounded-full ${selectedTeam.name === data.name ? "bg-[#4C0BFD]" : "bg-transparent"}`}
                  />
                </div>
      ...
      <div className="flex items-center justify-between w-full px-16 max-md:fixed max-md:bottom-5 max-md:left-0 max-md:right-0 max-md:px-4 max-md:py-3  z-50">
        ...
        <button
          disabled={isDisabled}
          className="bg-[#3702C8] disabled:bg-[#A4A4A4] text-white  max-md:w-full cursor-pointer disabled:cursor-not-allowed text-base font-medium px-8 py-3.5 rounded-lg"
          onClick={handleStepsAndVerify}
        >
          {currentStep?.step === 1 && (
